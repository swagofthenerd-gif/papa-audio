...
// confidently. So the search score decides nothing until year agreement and
// release type have both spoken.
//
// Returns null rather than a guess when nothing clears the score floor: an
// unmatched record says so, which is a fact, where a wrong match is a lie.
function pickReleaseGroup(searchJson, opts) {
  const o = opts || {}
  const want = String(o.title || '').trim()
  const folderYear = Number(o.year) > 0 ? Number(o.year) : null
  ...
  for (const rg of list) {
    ...
    const typeScore = primaryType === 'Album'
      ...
      : (primaryType === 'EP' ? 1 : 0)
    cands.push({
      id: rg.id, title: String(rg.title || ''), date, rgYear,
  ...
  cands.sort((a, b) =>
    ...
    b.score - a.score)
  const best = cands[0]
  // Confidence is about the TITLE, not the ranking: the ranking's own type
  // preference will happily steer a folder called "Live at Leeds" onto the
  // studio record, and the panel has to be able to say so.
  const titleOk = !want ||
    SH.tokenScore(SH.normKey(want), SH.normKey(best.title)) >= ALBUM_MATCH_MIN
  ...
  return {
    ...
    score: best.score,
    confidence: titleOk && yearOk ? 'firm' : 'loose',
  }
}
...
module.exports = {
  ...
  pickReleaseGroup, releaseGroupFacts, studioAlbums,
}
