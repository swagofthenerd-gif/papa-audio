...
test('album-info returns the documented shape for a hit, and caches it', async () => {
  ...
  assert.ok(h.store.map['albuminfo:v1:pink floyd::wish you were here'], 'the answer is cached')
})
...
test('album-info caches a clean miss — but never a thrown lookup', async () => {
  ...
  assert.ok('albuminfo:v1:nobody::nothing' in miss.store.map,
...
test('album-info reads the cache BEFORE the first throttled call', async () => {
  ...
  const h = albumEnv({
    throttleThrows: true,
    seed: { 'albuminfo:v1:pink floyd::wish you were here': { found: true, confidence: 'firm', title: 'Wish You Were Here' } },
  })
...
test('artist-releases with no MBID searches for one, caches it, then asks once more', async () => {
  const h = albumEnv({
    mb: p => (p.startsWith('/artist?') ? { artists: [{ id: 'FOUND-MBID' }] } : PORTISHEAD_RGS),
  })
  ...
  assert.ok(h.store.map['artistrgs:v1:FOUND-MBID'])
