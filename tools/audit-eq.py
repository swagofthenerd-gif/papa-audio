...
for p in store['presets']:
    ...
    # 5. Headroom: sink volume must match the satellite peak.
    sat_peak = max([0]+[g for band,g in zip(BANDS,p['gains']) if band >= X])
    vol = re.search(r'volume = ([0-9.]+)', b)
    want = round(10 ** (-round(sat_peak)/20), 4) if sat_peak > 0 else None
    got = float(vol.group(1)) if vol else None
    if want is None and got is not None: fail.append(f"{k}: unexpected volume {got}")
    elif want is not None and (got is None or abs(got-want) > 0.001):
        fail.append(f"{k}: volume {got} != expected {want} (sat peak +{sat_peak})")
    # 6. Sub trim must compensate the sub-vs-satellite peak difference.
    sub_peak = max([0]+[g for band,g in zip(BANDS,p['gains']) if band <= X])
    trim = 10 ** (-max(0, sub_peak - sat_peak)/20)
    if mx:
        g6 = float(dict(g)['6'])
        if abs(g6 - 1.0*trim) > 0.002:
            fail.append(f"{k}: LFE mixer gain {g6} != expected {round(trim,4)}")
        g1 = float(dict(g)['1'])
        if abs(g1 - S['subMixGain']*trim) > 0.002:
            fail.append(f"{k}: satellite mixer gain {g1} != expected {round(S['subMixGain']*trim,4)}")
...
try:
    ...
    for k in keys:
        ...
        if n and n < 6:
            fail.append(f"{k}: only {n}/6 outputs linked to hardware")
