...
function filtersFor(ch, gains) {
  const out = [{ label: 'bq_highpass', freq: hpFreq, gain: 0, q: 0.7, tag: 'hp' }]
  gains.forEach((g, i) => {
    if (g === 0) return
...
function moduleFor(key, voicing) {
  ...
  for (const ch of CHANNELS) {
    const chain = filtersFor(ch, voicing.gains)
...
const mods = Object.entries(VOICINGS).map(([k, v]) => moduleFor(k, v))
